...
st.markdown("<h1 style ='text-align: center; color: #722290;'>Histórico de disparos en Nueva York 🗽 💥 </h1>", unsafe_allow_html =True)
...
top_perp_num = round((df['perp_sex'].value_counts()/df['perp_sex'].value_counts().sum())[0],2)*100

top_vic_name = df['vic_sex'].value_counts().index[0]
top_vic_num = round((df['vic_sex'].value_counts()/df['vic_sex'].value_counts().sum())[0],2)*100
...
top_perp_num = round((df['perp_race'].value_counts()/df['perp_race'].value_counts().sum())[0],2)*100

top_vic_name = df['vic_race'].value_counts().index[0]
top_vic_num = round((df['vic_race'].value_counts()/df['vic_race'].value_counts().sum())[0],2)*100
...
top_perp_num = round((df['perp_age_group'].value_counts()/df['perp_age_group'].value_counts().sum())[0],2)*100

top_vic_name = df['vic_age_group'].value_counts().index[0]
top_vic_num = round((df['vic_age_group'].value_counts()/df['vic_age_group'].value_counts().sum())[0],2)*100
...
top_perp_num = round((df['boro'].value_counts()/df['boro'].value_counts().sum())[0],2)*100
...
top_perp_num = round((df['hour'].value_counts()/df['hour'].value_counts().sum())[0],2)*100
...
c1.markdown("<h3 style ='text-align: center; color: red;'>¿Dónde han ocurrido disparos en Nueva York?</h3>", unsafe_allow_html =True)
year = c1.slider('Año en el que ocurrió el suceso', int(df.year.min()), int(df.year.max()))
c1.map(df[df['year']==year][['latitude','longitude']])

c2.markdown("<h3 style ='text-align: center; color: red;'>¿A que horas ocurren disparos en Nueva York?</h3>", unsafe_allow_html =True)
...
st.markdown("<h3 style ='text-align: center; color: red;'>¿Cómo ha sido la evolución disparos por barrio en Nueva York?</h3>", unsafe_allow_html =True)
...
c4.markdown("<h3 style ='text-align: center; color: red;'>¿Qué edad tienen los atacantes?</h3>", unsafe_allow_html =True)
...
c5.markdown("<h3 style ='text-align: center; color: red;'>¿Qué edad tienen las victimas?</h3>", unsafe_allow_html =True)
...
c6.markdown("<h3 style ='text-align: center; color: red;'>¿Cuál es el sexo del atacante?</h3>", unsafe_allow_html =True)
...
c7.markdown("<h3 style ='text-align: center; color: red;'>¿Cuál es el sexo de la victima?</h3>", unsafe_allow_html =True)
...
st.markdown("<h3 style ='text-align: center; color: red;'>¿Evolución de disparos por año en las horas con más y menos sucesos?</h3>", unsafe_allow_html =True)
